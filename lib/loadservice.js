            ...
            if (!reloadOnlyInstances) {
                ...
                modules[modDesc.name].init(function register(o) {
                    try {
                        ...
                        logger.error("Could not load module " + modDesc.name + " with message: " + err);
                    }
...
exports.checkForWebinosModules = function(node_moduleDir, userDataPath) {
    try {
        ...
        for (i = 0; i < fileList.length; i = i + 1) {
            ...
            if ((stat.isDirectory() && (fileList[i].indexOf("webinos-api-") === 0 || fileList[i].indexOf("ubiapps-api-") === 0)) || fileList[i] === "webinos-utilities") {
                try{
...
exports.createWebinosJS = function(node_moduleDir, apiModules) {
    var webinosJSPath = path.join(wPath.webinosPath(), "wrt");
    var webinosJSFile = path.join(webinosJSPath, "webinos.js");
    var webinosJSStream = fs.createWriteStream(webinosJSFile, { flags:"w", encoding:"utf8"});
    webinosJSStream.on("open", function() {
      webinosJSStream.write("(function(exports){\n\n");

      // Pre-load mandatory client code.
      // Order is important - need to write first webinos.session and then webinos.js otherwise it fails
      var fileList = [
        ...
        path.join(node_moduleDir, "../wrt", "webinos.session.js"),
        path.join(node_moduleDir, "../wrt", "webinos.js"),
      ...
      // Include proxy files downloaded from connected PZPs
      var downloadedProxyFiles = fs.readdirSync(webinosJSPath);
      for (var j=0; j < downloadedProxyFiles.length; j = j + 1) {
        var stat = fs.statSync(path.join(webinosJSPath, downloadedProxyFiles[j]));
        if (stat.isFile() && path.extname(downloadedProxyFiles[j]) === ".js" && downloadedProxyFiles[j] != "webinos.js"){
          fileList.push(path.join(webinosJSPath,downloadedProxyFiles[j]));
        }
      }
      ...
      if (dashboard != null){
        fileList.push(path.join(node_moduleDir,"webinos-dashboard","wrt","webinos.dashboard.js"));
      }

      // Gather proxy code from api modules.
      for (var i = 0; i < apiModules.length; i = i + 1) {
        var apiProxyList = fs.readdirSync(path.join(apiModules[i].path, "wrt"));
        // Sort to allow modules to manage dependencies (see file api wrt files).
        apiProxyList.sort();
        for (j=0; j < apiProxyList.length; j = j + 1) {
          stat = fs.statSync(path.join(apiModules[i].path, "wrt", apiProxyList[j]));
          if (stat.isFile()) {
              if (path.extname(apiProxyList[j]) === ".js"){
                fileList.push(path.join(apiModules[i].path, "wrt", apiProxyList[j]));
              }
          }
        }
      }

      // Copy files to webinos.js stream.
      fileList.forEach(function(name){
        var data = fs.readFileSync(name);
        webinosJSStream.write(data.toString());
      });

      webinosJSStream.write("\n})(window);\n");
      webinosJSStream.end();
      ...
      if(require('os').platform() === 'android')
      {
        ...
        var androidDir = path.join(node_moduleDir, "../android");

        if (!existsSync (wrtDir))
          fs.mkdirSync (wrtDir);
        var staticPath = androidInterfaces.static;
        if (!existsSync (staticPath))
          fs.mkdirSync (staticPath);

        var webinossoketJs = path.join(androidDir,"wrt","webinossocket.js");
        var data = fs.readFileSync(webinossoketJs);
        fs.writeFileSync(path.join(staticPath, "webinos.js"), data);
      }
    });
};
