	...
	var Discovery = function(rpcHandler, params) {
		...
		this.findConfigurableAPIs = function (params, successCB, errorCB, objectRef) {
			...
			for (var i in modulesFolders) {
                ...
                if (modulesFolders[i] === "webinos-utilities") {
                    configFilePath = path.join(moduleBasePath, "node_modules", "webinos-api-test", "config.json");
                    userDataConfigFilePath = path.join(require("./webinosPath.js").webinosPath(), "userData", "webinos-utilities", "node_modules", "webinos-api-test", "config.json");
                } else {
                    configFilePath = path.join(moduleBasePath, "config.json");
                    userDataConfigFilePath = path.join(require("./webinosPath.js").webinosPath(), "userData", params[1].name, "config.json");
                }

                if (existsSync(templateFilePath))
                {
                    if (existsSync(userDataConfigFilePath)) {
                        config = require(userDataConfigFilePath);
                    } else if (existsSync(configFilePath)) {
                        config = require(configFilePath);
                    }
                    if (config && config.apiURI) {
                        installedAPIs.push({
                            'name': modulesFolders[i]
                            , 'path': moduleBasePath
                            , 'uri': config.apiURI
                        });
                    }
		...
		var search = function (serviceType, callback, options, filter) {
			...
			if (this.rpcHandler.parent && filter && filter.zoneId.length > 0 && filter.cache == false) {
