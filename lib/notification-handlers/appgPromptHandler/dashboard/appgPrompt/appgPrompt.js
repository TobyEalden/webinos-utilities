...
function setData(args){
    ...
    var txt = transaction.origin + " is requesting a payment of " + transaction.value + transaction.currency + " via " + transaction.source + " to " + transaction.destination + " for " + transaction.description;

    document.getElementById('newUserReqButtons').innerHTML = txt;
  ...
  var newUserReqDecline = document.getElementById('newUserReqDecline');
  newUserReqDecline.onclick = function() {
    doDecline();
  };

  var newUserReqClose = document.getElementById('newUserReqClose');
  newUserReqClose.onclick = function() {
    doClose();
  };
...
function doContinue() {
  webinos.dashboard.actionComplete({ responseTo: notifyId, confirmed: true });
  setTimeout(function() { window.close(); }, 500);
}
  
function doDecline() {
  webinos.dashboard.actionComplete({ responseTo: notifyId, declined: true });
  setTimeout(function() { window.close(); }, 500);
  }

function doClose() {
  webinos.dashboard.actionComplete({ responseTo: notifyId, confirmed: false });
  setTimeout(function() { window.close(); }, 500);
}