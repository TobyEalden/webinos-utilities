(function(exports) {
  ...
  var ActionHandler = function () {
    ...
    parent.receivePendingActions = function(receivedMsg) {
      ...
      console.log(parent.getSessionId() + " ******** received actions from " + entityId + " " + JSON.stringify(actions));
    ...
    function sendPendingActionsInternal(entityId) {
      ...
      if (pending.length > 0) {
        ...
        console.log(parent.getSessionId() + " ******** sending actions to " + entityId + " " + JSON.stringify(pending));
        sendMessage(entityId, "actionsReceivePending", pending);
      }
    }

    function actionsAcknowledgedInternal(entityId, ackList) {
      console.log(parent.getSessionId() + " ******** received acks from " + entityId + " " + JSON.stringify(ackList));
    ...
    function addActionInternal(actionData) {
      ...
      if (actionDb.actions.hasOwnProperty(actionData.id)) {
        console.log(parent.getSessionId() + " **************** action already exists: " + actionData.id);
      }
    ...
    function triggerSync() {
      ...
      if (syncPending === false) {
        ...
        console.log(parent.getSessionId() + " ************** triggerSync - sync already pending");
      }
    }
    ...
    function sendAcknowledgments(entityId,ackList) {
      console.log(parent.getSessionId() + " ******** sending acks to " + entityId + " " + JSON.stringify(ackList));
