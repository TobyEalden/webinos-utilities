...
ProcessWebinosMsg.readJson = function(instance, buffer, objectHandler) {
  ...
  // First just try parsing the buffer to JSON (ignoring the 4-byte length prefix).
  try {
    var bufferString = buffer.slice(4).toString("utf8");
    var obj = JSON.parse(bufferString);
    callback(obj);
  } catch (e) {
    for (; ;) {
      // Failed to parse => buffer isn't a complete JSON structure.
      // Attempt to concatenate consecutive chunks into a complete packet.
      var readByteLen;
      if (instance in instanceMap) {
        // Already have a chunk - read the next chunk and append it to the buffer.
