...
//                        contentsPzp[key] = jsonObject[key];
    ...
    this.sendObjectContents = function(jsonObject, receivedDiff) {
     ...
     function contains(localArr, lname) {
         for (var i = 0 ; i < localArr.length; i = i + 1) {
            if (JSON.stringify(localArr[i]) == JSON.stringify(lname)) {
              return true;
            }
         }
         return false;
    ...
    function isLocalAddress(str, ownId){
    ...
    function findDiffApply(remoteJson, localJson, ownId) {
        ...
        if (Object.prototype.toString.call(remoteJson) === "[object Array]" &&
            ...
            remoteJson.forEach(function(rname, rindex){
                if (rname /*&& rname.id*/) {
                    ...
                    localJson.forEach(function(lname, index){
                       if (JSON.stringify(rname) === JSON.stringify(lname)){//.api === lname.api && rname.serviceAddress === lname.serviceAddress) {
            ...
            localArr.sort(function(a,b) {return ((a.id)> ( b.id) ? 1 : ((b.id) > (a.id)) ? -1 : 0);} );
            return localArr;
            ...
            for (var key in remoteJson) {
                if (remoteJson.hasOwnProperty(key) && localJson && localJson.hasOwnProperty(key)){
                    if(typeof remoteJson[key] === "string") {
                        ...
                        localDiff[key]=  findDiffApply(remoteJson[key], localJson[key], ownId);
                    }
                } else {
            ...
            var sortable = [], localValue;
            for (key in localDiff)
                sortable.push(key);
            sortable.sort(function(a,b) {return ((a.id)> ( b.id) ? 1 : ((b.id) > (a.id)) ? -1 : 0);} );

            for (key = 0; key < sortable.length; key++){
                localValue = sortable[key];
                localDiff[localValue] = localDiff[localValue];
            }
        }
        return localDiff;
    }
     ...
     */
    this.applyObjectContents = function(localJson, remoteJsonObject, ownId) {
        ...
        remote.forEach(function(key){
            if(local.indexOf(key) === -1) { // This is new element add element, locally it does not exist
                ...
                remoteJsonObject[key] !== localJson[key]) { // Element are string
                    ...
                    localJson[key] = findDiffApply(remoteJsonObject[key], localJson[key], ownId);
                }
            }
        });
    };
