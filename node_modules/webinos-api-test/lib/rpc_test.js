...
TestApiModule.prototype.updateServiceParams = function (serviceId, params) {
    var id;

    if (serviceId && this.internalRegistry[serviceId]) {
        this.unregister({"id":serviceId, "api": this.internalRegistry[serviceId].api} );
        delete this.internalRegistry[serviceId];
    }

    if (params) {
        var service = new TestService(this.rpcHandler, params);
...
var TestService = function(rpcHandler, params) {
	...
	this.base({
		...
		displayName:'Test',
		description:'Test Module with the life answer... ' + this.blaa
	});
	...
	this.listenAttr.listenFor42 = function(params, successCB, errorCB, objectRef){
		...
		rpcHandler.executeRPC(rpc, successCB, errorCB);
	};
}
